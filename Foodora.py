...
def extract_amount(text):
    ...
    if match:
        ...
        return float(cleaned)  # Returnerer faktisk verdi, inkludert negative tall
    return None
    ...
    if sanctions:
        ...
        if sanctions_amount is not None:
            data.append({
                ...
                'Dato': dato.group(1) if dato else None,
                'Konto': '7210',
                'Beløp': abs(sanctions_amount)  # Bruker abs() for å få positiv verdi
            })
    if hardware:
        ...
        if hardware_amount is not None:
            data.append({
                ...
                'Dato': dato.group(1) if dato else None,
                'Konto': '6551',
                'Beløp': abs(hardware_amount)  # Bruker abs() for å få positiv verdi
            })
    return data
...
def main():
    ...
    # Ber brukeren velge hvor Excel-filen skal lagres
    ...
    # Viser en oppsummering av dataene
